...
export function Overlay({ configurations, onOpenSettings, onOpenHistory, providers }: {
  ...
  const [providerModels, setProviderModels] = useState<import("../../types/domain").ProviderModel[]>([]);
  ...
  useEffect(() => {
    try { localStorage.setItem("evoca.configurationView", configurationView); } catch { /* localStorage can be unavailable in restricted WebView contexts */ }
  }, [configurationView]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    evoca.getProviderModels(selected.providerId).then((models) => {
  ...
  const modelOptions = useMemo(() => {
    const options = providerModels.map((model) => ({ value: model.name, label: model.displayName || model.name }));
  ...
  async function run() {
    ...
    try { await evoca.startConfigurationStreamWithModel(selected.id, state.input, requestId, chatModel || selected.model); }
  ...
  if (state.state === "searching") return (
    <section className="h-full w-full overflow-hidden border border-evoca-line bg-[linear-gradient(180deg,rgba(18,21,27,.985),rgba(10,12,16,.995)),radial-gradient(circle_at_24%_0%,rgba(255,255,255,.04),transparent_34%)] shadow-[0_35px_100px_rgba(0,0,0,.58),0_12px_30px_rgba(0,0,0,.22),inset_0_1px_0_rgba(255,255,255,.035)] flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col px-5 pb-5">
        ...
        <div className="mb-3 flex items-center justify-between"><span className="text-[9px] font-medium uppercase tracking-[.15em] text-white/28">{filtered.length} configurations</span><div className="flex items-center gap-2"><small className="text-[9px] text-white/24">Choose one to start a run</small><div className="flex items-center rounded-[9px] border border-white/[.07] bg-white/[.025] p-0.5" role="group" aria-label="Configuration view"><button type="button" className={`rounded-[7px] px-2 py-1 text-[9px] font-semibold transition ${configurationView === "grid" ? "bg-white/[.09] text-white" : "text-white/30 hover:text-white/65"}`} aria-pressed={configurationView === "grid"} onClick={() => setConfigurationView("grid")}>Grid</button><button type="button" className={`rounded-[7px] px-2 py-1 text-[9px] font-semibold transition ${configurationView === "list" ? "bg-white/[.09] text-white" : "text-white/30 hover:text-white/65"}`} aria-pressed={configurationView === "list"} onClick={() => setConfigurationView("list")}>List</button></div></div></div>
  ...
  if (state.state === "loading") {
    ...
    return <section className="relative h-full w-full overflow-hidden border border-evoca-line bg-[linear-gradient(180deg,rgba(18,21,27,.985),rgba(10,12,16,.995)),radial-gradient(circle_at_24%_0%,rgba(255,255,255,.04),transparent_34%)] shadow-[0_35px_100px_rgba(0,0,0,.58),0_12px_30px_rgba(0,0,0,.22),inset_0_1px_0_rgba(255,255,255,.035)] flex h-full flex-col"><div className="flex min-h-14 items-center gap-2 border-b border-white/[.06] px-5 [--wails-draggable:drag]"><div className="flex min-w-0 flex-1 items-center gap-2.5"><div><h2 className="m-0 truncate text-[12px] font-semibold text-white">{selected?.name}</h2><div className="text-[9px] text-white/30">{chatModel || selected?.model || "Model"}</div></div></div></div>
      <div className="min-h-0 flex-1 px-5 py-4 pb-16">{state.output ? <div className="h-full min-h-[260px] overflow-auto rounded-[14px] border border-white/[.06] bg-[#0a0c10] p-4 leading-6 shadow-[inset_0_1px_0_rgba(255,255,255,.02)]"><Markdown source={state.output} /><span className="opacity-70 animate-[evoca-blink_1s_steps(2,start)_infinite]">▌</span></div> : <div className="flex h-full min-h-[280px] flex-col items-center justify-center gap-3 text-center"><span className="h-3 w-3 rounded-full bg-evoca-accent shadow-[0_0_24px_rgba(216,184,110,.3)] animate-[evoca-thinking_1.7s_ease-in-out_infinite]" aria-hidden="true" /><strong>{thinkingLabel}</strong><span className="text-[9px] text-white/24">{elapsedSeconds}s elapsed</span><span className="max-w-[260px] text-[9px] leading-5 text-white/25">eVoca is streaming the result as soon as the model produces visible output.</span><button type="button" className="mt-1 inline-flex items-center justify-center rounded-[10px] border border-red-200/10 bg-red-300/[.055] px-3 py-2 text-[10px] font-semibold text-evoca-danger transition hover:bg-red-300/[.1]" onClick={cancelCurrentRequest}>Cancel</button></div>}
        {streaming && state.output && <div className="flex items-center gap-2 border-t border-white/[.06] px-1 py-3 text-[9px] text-white/30"><span className="flex items-center gap-1"><i className="block h-1.5 w-1.5 rounded-full bg-white/45 animate-[evoca-pulse_1.2s_infinite_ease-in-out]"></i><i className="block h-1.5 w-1.5 rounded-full bg-white/45 animate-[evoca-pulse_1.2s_infinite_ease-in-out] [animation-delay:.15s]"></i><i className="block h-1.5 w-1.5 rounded-full bg-white/45 animate-[evoca-pulse_1.2s_infinite_ease-in-out] [animation-delay:.3s]"></i></span><span>Generating…</span></div>}{streaming && state.output && <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 flex items-center justify-end border-t border-white/[.06] bg-[#0a0c10]/95 px-5 py-3 shadow-[0_-8px_24px_rgba(0,0,0,.18)] backdrop-blur-sm"><button type="button" className="pointer-events-auto rounded-[10px] border border-red-200/15 bg-red-300/[.07] px-3 py-2 text-[9px] font-semibold text-red-50 transition hover:bg-red-300/[.12]" onClick={cancelCurrentRequest}>Stop generating</button></div>}</div></section>;
  }

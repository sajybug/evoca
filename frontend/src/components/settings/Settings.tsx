...
const settingsSectionMetaClass =
  'text-[9px] font-semibold uppercase tracking-[.14em] text-white/25';
...
export function Settings({
  ...
  const [confirm, setConfirm] = useState<{
    kind: 'configuration' | 'provider' | 'model' | 'restore';
    id?: string;
  } | null>(null);
    ...
    evoca
      ...
      .then((data) => {
        ...
        setConfiguration((current) => {
          ...
          if (current.model && nextModels.some((model) => model && model.name === current.model))
            return current;
  ...
  async function saveProviderCredential() {
    ...
    try {
      ...
      setMessage(`Store credential failed: ${String(error)}`);
    } finally {
  ...
  async function removeProviderCredential() {
    ...
    try {
      ...
      setMessage(`Remove credential failed: ${String(error)}`);
    } finally {
      setCredentialBusy(false);
    }
  }
    ...
    <section className='h-full w-full overflow-hidden border border-evoca-line bg-[linear-gradient(180deg,rgba(18,21,27,.985),rgba(10,12,16,.995)),radial-gradient(circle_at_24%_0%,rgba(255,255,255,.04),transparent_34%)] shadow-[0_35px_100px_rgba(0,0,0,.58),0_12px_30px_rgba(0,0,0,.22),inset_0_1px_0_rgba(255,255,255,.035)] flex flex-col'>
      <div className='flex items-center justify-between border-b border-white/[.06] px-5 py-4 [--wails-draggable:drag]'>
        <div className='flex items-center gap-3'>
          <span className='grid h-8 w-8 shrink-0 place-items-center rounded-[11px] border border-evoca-accent/20 bg-[#F5E8C5] text-[30px] text-[#1B1B1A] inline-flex items-center justify-center leading-none'>
            ✦
          </span>
          <div>
            <h1 className={settingsTitleClass}>Settings</h1>
            <p className='text-[9px] leading-4 text-white/32'>
              Shape how eVoca runs, stores data, and connects to models.
            </p>
          </div>
        </div>
        <div className='flex items-center gap-2'>
          <button
            type='button'
            className='inline-flex items-center gap-1.5 rounded-[9px] border border-white/[.07] bg-white/[.03] px-2.5 py-1.5 text-[9px] font-semibold text-white/60 transition hover:bg-white/[.06] hover:text-white !border-red-200/10 !bg-red-300/[.04] !text-red-100/60 hover:!bg-red-300/[.08] hover:!text-red-50'
      ...
      <div className='mx-5 my-4 grid grid-cols-4 gap-1 rounded-[12px] border border-white/[.06] bg-black/15 p-1'>
        ...
        <button
          type='button'
          className={`${section === 'configurations' ? 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70 bg-white/[.075] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.04)]' : 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70'}`}
        ...
        <button
          type='button'
          className={`${section === 'providers' ? 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70 bg-white/[.075] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.04)]' : 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70'}`}
        ...
        <button
          type='button'
          className={`${section === 'backup' ? 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70 bg-white/[.075] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.04)]' : 'rounded-[9px] border-0 bg-transparent px-3 py-2 text-[10px] font-semibold text-white/35 transition hover:bg-white/[.035] hover:text-white/70'}`}
      ...
      <div className='relative min-h-0 flex-1 overflow-hidden px-5 pb-5'>
        {message && (
          <div
            className={`pointer-events-none absolute left-5 right-5 z-50 rounded-[10px] border px-3 py-2 text-[9px] leading-4 shadow-lg backdrop-blur-md transition-all duration-200 ${message.toLowerCase().includes('successful') || message.toLowerCase().includes('saved') || message.toLowerCase().includes('changed') || message.toLowerCase().includes('found') || message.toLowerCase().includes('added') ? 'border-emerald-200/10 bg-emerald-300/[.04] text-evoca-success' : 'border-red-200/10 bg-red-300/[.04] text-evoca-danger'}`}
          >
            {message}
          </div>
        )}
        ...
        {section === 'general' ? (
          <div className='h-full min-h-0 overflow-y-auto pr-1'>
            <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4'>
              <div className='mb-4 flex items-start justify-between gap-3'>
                <div>
                  <h3 className={settingsHeadingClass}>General</h3>
                  <p className='mt-1.5 text-[9px] leading-4 text-white/28'>
                    Global behavior and local storage for this installation.
                  </p>
                </div>
                <span className={settingsBadgeClass}>LOCAL</span>
              </div>
              <div className='rounded-[14px] border border-white/[.06] bg-white/[.018] p-4'>
                <div className='mb-3 flex items-center justify-between'>
                  <div>
                    <div className={settingsSectionMetaClass}>Global hotkey</div>
                    <p className={settingsBodyClass}>
                      Toggle the launcher from anywhere in Windows.
                    </p>
                  </div>
                </div>
                <div className='grid grid-cols-2 gap-3'>
                  <label className={settingsLabelClass}>
                    ...
                    />
                    <span className='mt-1.5 text-[8px] leading-4 text-white/23'>
                      Escape dismisses the transient launcher views.
                    </span>
                  </label>
                  <div className='mt-5 my-auto flex items-center justify-between rounded-[10px] border border-white/[.05] bg-black/15 px-3 py-2.5 self-end text-[9px] text-white/35'>
                    <span>Current</span>
                    <code className='rounded-[6px] border border-white/[.08] bg-white/[.025] px-1.5 py-1 font-mono text-[8px] text-white/33'>
                      {hotkey}
                    </code>
                  </div>
                </div>
              ...
              <div className='mt-5 border-t border-white/[.06] pt-4'>
                <div className='mb-3 flex items-center justify-between'>
                  <div>
                    <div className={settingsSectionMetaClass}>Data storage</div>
                    <p className={settingsBodyClass}>
                      SQLite database and captured screenshot files.
                    </p>
                  </div>
                  <span className='text-[8px] text-white/22'>Restart after path changes</span>
                </div>
                <div className='rounded-[14px] border border-white/[.06] bg-white/[.018] p-4'>
                  <p className='mb-4 text-[10px] leading-5 text-white/35'>
                    Choose where eVoca keeps its local database and chat images. Changing these
                    paths is persisted immediately, but the new database location is picked up after
                    restart.
                  </p>
                  {storage && (
                    <div className='space-y-3'>
                      <label className={settingsLabelClass}>
                        ...
                        <div className='flex items-center gap-2'>
                          <input
                            readOnly
                            className='min-w-0 flex-1 rounded-[11px] border border-white/[.075] bg-white/[.025] px-3 py-2.5 text-[11px] text-white/65 outline-none'
                            value={storage.databasePath}
                          />
                          <button
                            type='button'
                            ...
                            onClick={async () => {
                              try {
                                const selected = await evoca.chooseDirectory(
                                  storage.databasePath,
                                  'Choose database folder',
                                );
                                if (selected) {
                                  const base = selected.replace(/[\\/]+$/, '');
                                  setStorage({ ...storage, databasePath: `${base}\\evoca.db` });
                                }
                              } catch (error) {
                                setMessage(`Folder selection failed: ${String(error)}`);
                              }
                            }}
                          >
                            Choose…
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                const selected = await evoca.chooseDirectory(
                                  storage.imagesPath,
                                  'Choose chat images folder',
                                );
                                if (selected) setStorage({ ...storage, imagesPath: selected });
                              } catch (error) {
                                setMessage(`Folder selection failed: ${String(error)}`);
                              }
                            }}
                      ...
                      <div className='mt-4 flex items-center justify-between gap-3 border-t border-white/[.06] pt-4'>
                        <small className='text-[8px] text-white/25'>
                          Local files only · no account required
                        </small>
                        <button
                          type='button'
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setMessage(
                                'Storage paths saved. Restart eVoca to apply the new database path.',
                              );
                            } catch (error) {
                        ...
                        >
                          {storageSaving ? 'Saving…' : 'Save paths'}
                        </button>
                      </div>
                    </div>
                  )}
          ...
          <div className='h-full min-h-0 overflow-y-auto pr-1'>
            <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4'>
              <div className='mb-4 flex items-start justify-between gap-3'>
                <div>
                  <h3 className={settingsHeadingClass}>Backup & Restore</h3>
                  <p className='mt-1.5 text-[9px] leading-4 text-white/28'>
                    Choose between a complete local backup or settings only.
                  </p>
                </div>
                <span className={settingsBadgeClass}>LOCAL</span>
              </div>
              <div className='space-y-3'>
                <div className='rounded-[14px] border border-white/[.06] bg-white/[.018] p-4'>
                  ...
                  <p className={settingsBodyClass}>
                    Choose what to include. API keys are never copied from Windows Credential
                    Manager or environment variables.
                  </p>
                  <div className='mt-3 grid grid-cols-2 gap-3 max-[760px]:grid-cols-1'>
                    <button
                      type='button'
                      className='rounded-[12px] border border-white/[.075] bg-white/[.025] p-3 text-left transition hover:border-evoca-accent/30 hover:bg-white/[.05] disabled:opacity-40'
                      disabled={backupBusy}
                      onClick={async () => {
                        setBackupBusy(true);
                        try {
                          const target = await evoca.chooseBackupSavePath('');
                          if (target) {
                            await evoca.createBackup(target, 'full');
                            setMessage('Full backup created successfully.');
                          }
                        } catch (error) {
                          setMessage(`Full backup failed: ${String(error)}`);
                        } finally {
                          setBackupBusy(false);
                        }
                      }}
                    >
                      ...
                      <div className='mt-1 text-[9px] leading-4 text-white/30'>
                        Database, History images, providers, configurations, and local settings.
                      </div>
                    </button>
                    <button
                      type='button'
                      className='rounded-[12px] border border-evoca-accent/25 bg-evoca-accent/[.05] p-3 text-left transition hover:border-evoca-accent/40 hover:bg-evoca-accent/[.08] disabled:opacity-40'
                      disabled={backupBusy}
                      onClick={async () => {
                        setBackupBusy(true);
                        try {
                          const target = await evoca.chooseBackupSavePath('');
                          if (target) {
                            await evoca.createBackup(target, 'settings');
                            setMessage('Settings backup created successfully.');
                          }
                        } catch (error) {
                          setMessage(`Settings backup failed: ${String(error)}`);
                        } finally {
                          setBackupBusy(false);
                        }
                      }}
                    >
                      <div className='text-[10px] font-semibold text-evoca-accent'>
                        Settings only
                      </div>
                      <div className='mt-1 text-[9px] leading-4 text-white/30'>
                        Providers, models, and configurations only. History stays untouched.
                      </div>
                    </button>
                  </div>
                </div>
                <div className='rounded-[14px] border border-white/[.06] bg-white/[.018] p-4'>
                  <div className='flex items-start justify-between gap-4'>
                    <div>
                      <div className={settingsSectionMetaClass}>Restore backup</div>
                      <p className={settingsBodyClass}>
                        eVoca detects the backup type automatically. Full backups replace local
                        database data and History images; settings backups replace only providers,
                        models, and configurations.
                      </p>
                    </div>
                    <button
                      type='button'
                      className='shrink-0 inline-flex items-center justify-center gap-1.5 rounded-[10px] border border-white/[.075] bg-white/[.045] px-3 py-2 text-[10px] font-semibold text-white/82 transition hover:border-white/[.12] hover:bg-white/[.08] hover:text-white disabled:opacity-40 !border-red-200/10 !bg-red-300/[.055] !text-evoca-danger'
                      disabled={backupBusy}
                ...
                <div className='rounded-[12px] border border-amber-200/[.08] bg-amber-200/[.025] px-3 py-2.5 text-[9px] leading-5 text-white/35'>
                  Full backups restore the database and History images but never restore machine
                  secrets. Settings-only restore leaves History, images, and app-level settings
                  untouched.
                </div>
              </div>
            </div>
          ...
          <div className='grid min-h-0 h-full grid-cols-[228px_minmax(0,1fr)] gap-5 max-[900px]:grid-cols-1'>
            <aside className='min-h-0 overflow-auto border-r border-white/[.06] pr-4 max-[900px]:max-h-[240px] max-[900px]:border-r-0 max-[900px]:border-b max-[900px]:pb-3 max-[900px]:pr-0'>
              <div className='mb-2 flex items-center justify-between text-[9px] font-medium uppercase tracking-[.12em] text-white/25'>
                <span>Configurations</span>
                <span>{configurations.length}</span>
              </div>
              <button
                type='button'
                className='mb-1 w-full rounded-[11px] border border-transparent bg-transparent px-3 py-2.5 text-left transition hover:border-white/[.06] hover:bg-white/[.04] mb-3 border-dashed !border-evoca-accent/20 !bg-evoca-accent/[.025] !text-evoca-accent hover:!bg-evoca-accent/[.05]'
                onClick={() => setConfiguration(freshConfiguration(providers))}
              >
                <span className='flex items-center gap-2 truncate text-[10px] font-semibold text-white/80'>
                  <span className='grid h-7 w-7 shrink-0 place-items-center rounded-[8px] border border-white/[.06] bg-white/[.03] text-[10px] text-evoca-accent'>
                    ＋
                  </span>
                  New configuration
                </span>
              ...
              {configurations.map((s) => (
                <button
                  ...
                  key={s.id}
                  className={
                    configuration?.id === s.id
                      ? 'mb-1 w-full rounded-[11px] border border-transparent bg-transparent px-3 py-2.5 text-left transition hover:border-white/[.06] hover:bg-white/[.04] border-evoca-accent/30 bg-evoca-accent/[.08] shadow-[inset_2px_0_0_rgba(216,184,110,.65),0_0_0_1px_rgba(216,184,110,.04)]'
                ...
                >
                  ...
                  <small className='text-xs mt-0.5 block truncate !text-white/24'>
                    {providers.find((p) => p.id === s.providerId)?.name || 'No provider'} ·{' '}
                    {s.model || 'No model'}
                  </small>
                </button>
              ))}
            ...
            <div className='min-w-0 min-h-0 overflow-auto'>
              {configuration ? (
                <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4'>
                  <div className='mb-4 flex items-start justify-between gap-3'>
                    <div>
                      <h3 className={settingsHeadingClass}>
                        {configuration.name || 'Configuration'}
                      </h3>
                      <p className='mt-1.5 text-[9px] text-white/28'>
                        A reusable workflow from input to model result.
                      </p>
                    </div>
                    <span className={settingsBadgeClass}>CONFIGURATION</span>
                  </div>
                  <div className='grid grid-cols-2 gap-3'>
                    <label className={settingsLabelClass}>
                      Name
                    ...
                    <label className={settingsLabelClass}>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setConfiguration({ ...configuration, description: e.target.value })
                        }
                      />
                    </label>
                  </div>
                    ...
                    <label className={settingsLabelClass}>
                      ...
                      <SearchableSelect
                        ...
                        options={[
                          ...
                          ...providers.map((r) => ({ value: r.id, label: r.name })),
                        ]}
                        onChange={(providerId) => {
                    ...
                    <label className={settingsLabelClass}>
                      ...
                      <SearchableSelect
                        ...
                        options={[
                          ...
                          ...(configuration.model &&
                          !configurationModels.some((m) => m.name === configuration.model)
                            ? [
                                {
                                  value: configuration.model,
                                  label: `${configuration.model} (saved)`,
                                },
                              ]
                            : []),
                        ...
                        onChange={(model) => setConfiguration({ ...configuration, model })}
                      />
                    </label>
                  </div>
                  <label className={settingsLabelClass}>
                    System Prompt
                    <textarea
                      className={`${settingsTextareaClass} min-h-[240px]`}
                      value={configuration.spell}
                      onChange={(e) =>
                        setConfiguration({ ...configuration, spell: e.target.value })
                      }
                    />
                    <span className='mt-1.5 text-[8px] leading-4 text-white/23'>
                      This prompt is sent as the reusable system instruction for every run.
                    </span>
                  </label>
                  <div className='mt-5 border-t border-white/[.06] pt-4'>
                    <div className='mb-3 flex items-center justify-between'>
                      <div>
                        <div className={settingsSectionMetaClass}>Generation</div>
                        <p className={settingsBodyClass}>
                          Keep these values close to the task rather than the provider.
                        </p>
                      </div>
                    </div>
                    <div className='grid grid-cols-2 gap-3'>
                  ...
                  <div className='mt-4 flex items-center justify-between gap-3 border-t border-white/[.06] pt-4'>
                    <div className='flex items-center gap-2'>
                      <button
                        type='button'
                        className='inline-flex items-center justify-center gap-1.5 rounded-[10px] border border-white/[.075] bg-white/[.045] px-3 py-2 text-[10px] font-semibold text-white/82 transition hover:border-white/[.12] hover:bg-white/[.08] hover:text-white'
                        onClick={() => void togglePinnedConfiguration()}
                      >
                        {configuration.pinned ? 'Unpin' : 'Pin'}
                      </button>
                      <button
                        type='button'
                        className='inline-flex items-center justify-center gap-1.5 rounded-[10px] border border-white/[.075] bg-white/[.045] px-3 py-2 text-[10px] font-semibold text-white/82 transition hover:border-white/[.12] hover:bg-white/[.08] hover:text-white'
                        onClick={() => void duplicateConfiguration()}
                      >
                        Duplicate
                      </button>
                      <button
                        type='button'
                        className='inline-flex items-center justify-center gap-1.5 rounded-[10px] border border-white/[.075] bg-white/[.045] px-3 py-2 text-[10px] font-semibold text-white/82 transition hover:border-white/[.12] hover:bg-white/[.08] hover:text-white disabled:opacity-40 !border-red-200/10 !bg-red-300/[.055] !text-evoca-danger hover:!bg-red-300/[.09]'
                        onClick={() => setConfirm({ kind: 'configuration', id: configuration.id })}
                      >
                        Delete
                      </button>
                      <div className='flex items-center gap-2'>
                ...
                <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4 flex min-h-[160px] flex-1 items-center justify-center text-center text-[10px] text-white/28'>
                  Create a configuration from the left panel to begin.
                </div>
              )}
            </div>
          </div>
            ...
            <aside className='min-h-0 overflow-auto border-r border-white/[.06] pr-4 max-[900px]:max-h-[240px] max-[900px]:border-r-0 max-[900px]:border-b max-[900px]:pb-3 max-[900px]:pr-0'>
              ...
              {providers.map((r) => (
                ...
                >
                  ...
                  <small className='mt-0.5 block text-[8px] text-white/25'>
                    {r.kind === 'ollama' ? 'Ollama' : 'OpenAI compatible'}
                  </small>
                </button>
              ))}
            </aside>
            <div className='min-w-0 min-h-0 overflow-auto'>
              {provider ? (
                <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4'>
                  <div className='mb-4 flex items-start justify-between gap-3'>
                    <div>
                      <h3 className={settingsHeadingClass}>{provider.name}</h3>
                      <p className='mt-1.5 text-[9px] text-white/28'>
                        Connection details, credentials references, and models.
                      </p>
                    </div>
                    <span className={settingsBadgeClass}>
                      {providers.some((item) => item.id === provider.id) ? 'SAVED' : 'NEW'}
                    </span>
                  </div>
                  ...
                  <div className='grid grid-cols-2 gap-3'>
                    ...
                    <label className={settingsLabelClass}>
                      ...
                      <SearchableSelect
                        ...
                        onChange={(kind) => setProvider({ ...provider, kind })}
                      />
                    </label>
                  </div>
                  <label className={settingsLabelClass}>
                  ...
                  <div className='mt-3 rounded-[12px] border border-evoca-accent/10 bg-evoca-accent/[.035] p-3'>
                    <div className='flex items-center justify-between gap-3'>
                      ...
                      <span
                        className={`text-[8px] font-semibold uppercase tracking-[.12em] ${credentialSaved ? 'text-evoca-success' : 'text-white/25'}`}
                      >
                        {credentialSaved ? 'STORED' : 'NOT STORED'}
                      </span>
                    </div>
                  ...
                  <label className={settingsLabelClass}>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setProvider({ ...provider, headersJson: e.target.value })}
                    />
                  </label>
                  ...
                  <div className='mt-5 border-t border-white/[.06] pt-4'>
                    <div className='mb-3 flex items-center justify-between'>
                      <div>
                        <div className={settingsSectionMetaClass}>Models</div>
                        <p className={settingsBodyClass}>
                          Add local model aliases or import what the provider exposes.
                        </p>
                      </div>
                      <span className='text-[8px] text-white/22'>{models.length} saved</span>
                    </div>
                    <div className='grid grid-cols-[1.15fr_1fr_auto] gap-2'>
                      <input
                        className='w-full rounded-[11px] border border-white/[.075] bg-white/[.025] px-3 py-2.5 text-[11px] text-white outline-none transition shadow-[inset_0_1px_0_rgba(255,255,255,.02)] focus:border-evoca-accent/35 focus:bg-white/[.045] focus:ring-2 focus:ring-evoca-accent/[.06] placeholder:text-white/25'
                    ...
                    <div className='mt-2 flex flex-col gap-1.5'>
                      {models.filter(Boolean).map((m) => (
                        <div
                          className='grid grid-cols-[1fr_1fr_auto] items-center gap-2 rounded-[10px] border border-white/[.05] bg-white/[.02] px-2.5 py-2.5'
                          key={m.id}
                        >
                          <span className='truncate text-[9px] text-white/65'>
                            {m.displayName?.trim() || m.name}
                          </span>
                          <code className='truncate rounded-[6px] border border-white/[.08] bg-white/[.025] px-1.5 py-1 font-mono text-[8px] text-white/33'>
                            {m.name}
                          </code>
                          <button
                            type='button'
                            className='rounded-[8px] border border-transparent px-2 py-1.5 text-[9px] text-white/35 transition hover:bg-white/[.05] hover:text-white'
                            onClick={() => setConfirm({ kind: 'model', id: m.id })}
                          >
                            Remove
                          </button>
                        </div>
                    ...
                    {discoveredModels.length > 0 && (
                      <div className='mt-4 rounded-[11px] border border-dashed border-white/[.06] bg-black/10 p-2.5'>
                        <div className='mb-2 text-[9px] font-semibold uppercase tracking-[.14em] text-white/25'>
                          ...
                          {discoveredModels.map((m) => (
                            ...
                            >
                              ...
                              <button
                                type='button'
                                className='rounded-[8px] border border-transparent px-2 py-1.5 text-[9px] text-white/35 transition hover:bg-white/[.05] hover:text-white'
                                onClick={() => void addDiscoveredModel(m)}
                              >
                                Add
                              </button>
                            </div>
                  ...
                  <div className='mt-4 flex items-center justify-between gap-3 border-t border-white/[.06] pt-4'>
                    <button
                      type='button'
                      className='inline-flex items-center justify-center gap-1.5 rounded-[10px] border border-white/[.075] bg-white/[.045] px-3 py-2 text-[10px] font-semibold text-white/82 transition hover:border-white/[.12] hover:bg-white/[.08] hover:text-white disabled:opacity-40 !border-red-200/10 !bg-red-300/[.055] !text-evoca-danger hover:!bg-red-300/[.09]'
                      onClick={() => setConfirm({ kind: 'provider', id: provider.id })}
                    >
                      Delete provider
                    </button>
                    <button
                ...
                <div className='rounded-[16px] border border-white/[.06] bg-white/[.018] p-4 flex min-h-[160px] flex-1 items-center justify-center text-center text-[10px] text-white/28'>
                  Add a provider from the left panel to connect eVoca to an LLM backend.
                </div>
              )}
            </div>
          </div>
      ...
      <ConfirmModal
        ...
        message={
          confirm?.kind === 'restore'
            ? 'eVoca will detect the backup type. A full backup replaces local database data and History images; a settings backup replaces providers, models, and configurations.'
        ...
        onConfirm={async () => {
          ...
          if (action.kind === 'restore') {
            ...
            try {
              ...
              setMessage(`Restore failed: ${String(error)}`);
            } finally {
              setBackupBusy(false);
            }
            return;
          ...
          if (action.kind === 'configuration' && action.id) {
            try {
              await evoca.deleteConfiguration(action.id);
          ...
          if (action.kind === 'provider' && action.id) {
            await performDeleteProvider();
            setConfirm(null);
            return;
          }
          if (action.kind === 'model' && action.id) {
            await performDeleteModel(action.id);
            setConfirm(null);
          }
        }}
      />
